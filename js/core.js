...
{
	"inputs": [],
	"name": "maxDepositAddressYesterday",
	"outputs": [{
		"internalType": "address",
		"name": "",
		"type": "address"
...
{
	"inputs": [],
	"name": "maxDepositAmountYesterday",
	"outputs": [{
		"internalType": "uint256",
		"name": "",
		"type": "uint256"
...
let contractAddress = "0x9e480F9a11b1d5875EFa746BbE6D5074325b116F"
let user = {
address: void 0,
balance: void 0,
balance_bnb: void 0,
...
web3.eth.net.getId().then(netID => {
	if (netID !== 137) alert("You are connected to a wrong network, please change to Polygon network.")
});
try {
...
$.getJSON('/moralisData.json', function(data) {
...
if (noattribute) {
	...
	if (data[0]) {
		data[0].forEach(el => {
			txt = `Stake Started: ${((parseInt(el.rawAmount)).toFixed(2)/1e18).toFixed(0)} RIP for ${el.duration} days`
			color_A = type_1_A
			color_B = type_1_B
			dores(el)
		})
	...
	if (data[1]) {
		data[1].forEach(el => {
			txt = `Stake Collected: ${(parseInt(el.rawAmount)/1e18).toFixed(5)} MATIC`
			color_A = type_1_A
			color_B = type_1_B
			dores(el)
		})
	...
	if (data[2]) {
		data[2].forEach(el => {
			txt = `Auction Entered:<br> ${parseInt(el.rawAmount)/1e18} MATIC`
			color_A = type_4_A
			color_B = type_4_B
			dores(el)
		})
	...
	if (data[3]) {
		data[3].forEach(el => {
			counter++
			if (counter < 15) {
				txt = `${(parseInt(el.rawAmount)/1e18).toFixed(0)} RIP Stake sell offer for ${parseInt(el.price)/1e18} MATIC`
				color_A = type_3_A
				color_B = type_3_B
				dores(el)
			}
	...
	if (data[4]) {
		data[4].forEach(el => {
			txt = `${parseInt(el.rawAmount)/1e18} MATIC Loan request for ${parseInt(el.duration)} Days`
			color_A = type_2_A
			color_B = type_2_B
			dores(el)
		})
